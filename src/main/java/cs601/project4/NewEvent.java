...
public class NewEvent extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			status = CommonServer.db.updateEventTable(email, eventName, category, location, description, doe,
					maxTickets, contact, time);
		...
		if (!status) {
			...
			String tempbody = "<form action=\"/logindisplay\" method=\"get\">" + "<br>"
					+ "<input type =\"hidden\" name=\"username\" value=" + email + ">"
					+ "<input type=\"submit\" value=\"Back\">" + "</form>" + "<form action=\"/logout\" method=\"get\">"
			...
			PrintWriter out = response.getWriter();
			out.println(
					"<html><title>newevent</title><body>The Event is not created. Please try again later.<br/>Thanks for visiting</body>");
			String tempbody = "<form action=\"/logindisplay\" method=\"get\">" + "<br>"
					+ "<input type =\"hidden\" name=\"username\" value=" + email + ">"
					+ "<input type=\"submit\" value=\"Back\">" + "</form>" + "<form action=\"/logout\" method=\"get\">"
					+ "<br>" + "<input type=\"submit\" value=\"Logout\">" + "</form>";
